...
# install OpenGL
RUN apt-get update && apt-get install -y build-essential libgl1-mesa-dev libglu1-mesa-dev freeglut3-dev libxi-dev
...
# Fetch OpenCV
RUN cd /opt && git clone --verbose https://github.com/opencv/opencv.git -b 4.5.3 &&\
    cd /opt && wget https://github.com/opencv/opencv_contrib/archive/4.5.3.tar.gz &&\
